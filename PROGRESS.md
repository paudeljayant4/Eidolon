...
## Cycle 7: Feature Depth — Building Resource Production/Consumption (Sep 2026)

### Observed
Buildings existed in the world (generated by `generate_cities`) but had empty `resourcesProduced`/`resourcesConsumed` fields. The `_process_resources` method only handled natural resource regrowth, not building production. Agents could `build` but buildings had no economic effect.

### Fix
1. **Populated `resourcesProduced`/`resourcesConsumed`** in `generate_cities` based on building type:
   ...
   - MARKET/BARRACKS/TEMPLE/PALACE: various or none

2. **Added `buildings` key** to world dict via `generate_cities` returning `(cities, buildings)` tuple

3. **Extended `_process_resources`** to process building production/consumption each tick, emitting `building_production` and `building_consumption` events

4. **Added `_handle_building_production`** for replay compatibility

5. **Updated `_handle_build`** to work with `world["buildings"]`
...
- Building production verified: 3 farms produce food every tick, consume water
...
- docs/issues/Cycle7-feature-depth.md: issue doc
- PROGRESS.md: this entry

### Category Rotation
Feature depth delivered. Next cycle could address: Correctness (verify no regressions), Performance (optimize building production), or Feature depth (economy chain - agents working buildings, resource transport).

## Cycle 8: Emergent Depth — Agent Decision Loop & Economy Integration (Sep 2026)

### Observed
After Cycle 7 added building production, the simulation had functioning resource flows (farms produce food, consume water) but the agent decision loop was disconnected from the simulation tick. Agents never acted in the main tick loop (`_process_agents` only emitted motivation events), causing:
- Zero trade/socialize/build events over 500 ticks
- Agents starving amid massive food surplus (37K→57K food)
- Water crisis: wells produced water but agents couldn't acquire it (`seek_water` only emitted motivation)
- Energy depletion: agents exhausted energy building without resting
- Social isolation: social need decayed to 0 with zero socialize events

### Fix
1. **Agent decision loop integrated into tick**: `_process_agent_decisions` now calls `perceive()` → `decide()` → `act()` for each agent each tick
2. **Water/thirst system completed**: Added `drink` action (consumes water, restores thirst/health/energy), `seek_water` now acquires water from resources/markets, wells produce water
3. **Planner priorities reordered**: Socialize (social < 0.5) before trade/build; Rest (rest < 0.3, energy < 0.5) before seek_water/drink; Build requires energy > 0.3
4. **New `drink` action**: Consumes water, restores thirst (+0.2), health (+0.1), energy (+0.1)
5. **Energy balance fixes**: Build cost reduced (0.2→0.15), rest energy gain increased (0.1→0.15), eat/drink energy gain increased (0.05→0.1)
6. **Market initialization**: Markets start with supplies/demands for active pricing
7. **Water resources**: Every region now has base water; wells produce water

### Verification
- **All 30 tests pass** (10 eat + 13 emergent + 2 market + 3 building + 2 social fragility)
- **Socialize events**: 25 over 500 ticks (starting tick 250, 5 per 50-tick window), social need stabilizes at 0.5
- **Trade events**: 15 total (10 in first 50 ticks, 5 in 50-99)
- **Build events**: 25 (reduced from 100 due to energy check)
- **Water production**: 14K→38K over 500 ticks
- **Determinism**: Same seed → identical results
- **Energy**: Improved but still depletes to 0 by tick 500 (known issue, see below)

### Remaining Issues
- **Energy depletion**: Agents still exhaust energy by tick 500 due to insufficient rest frequency vs build/decay costs
- **Trade sustainability**: Trade stops after tick ~100 as hunger stabilizes above 0.8 threshold
- **No org growth**: Single org persists, no new formations
- **Partner diversity**: All trade/socialize events involve same partner pairs

### Files Changed
- agents/_agent.py: decision loop, planner priorities, drink action, energy balances
- simulation/core.py: _process_agent_decisions, rest energy gain, build energy cost, _decay_needs energy
- simulation/world_gen.py: water resources per region, market supplies/demands, well building type
- simulation/_types.py: WELL building type
- test_emergent_depth.py: 3 new regression tests (socialize, trade, build)
- docs/issues/Cycle8-emergent-depth.md: issue doc
- docs/decisions/002-cycle8-emergent-depth.md: decision record
- PROGRESS.md: this entry
...
## Cycle 11: Feature Depth — Organization Growth (Sep 2026)

### Observed
Only 1 organization existed (Main Guild with agent-0). No agents joined or formed new orgs despite having form_organization and join_organization actions. The triggers were never reached because higher-priority actions (gather_wood, trade) always fired first.

### Evidence
| Metric | Before fix | After fix |
|--------|-----------|-----------|
| Agents with org | 1 | 5 |
| Org members | ['agent-0'] | all 5 agents |
...
4. Org members list updated in act() directly
5. Org info passed in needs dict so planners can see nearby agents' affiliations

### Verification
- All 43 tests pass (10 eat + 13 emergent + 2 market + 3 building + 2 social + 10 Cycle 9 + 3 Cycle 11)
- Determinism preserved
- All prior metrics unchanged (trade=1996, fed=120, drank=115, build=10, social=25)
- 3 new regression tests for org growth
