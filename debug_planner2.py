...
from simulation._types import ResourceType
...
for tick in range(100):
    ...
    if agent_actions:
        print(f"Tick {tick}: planner={decision.action}, actual={[f'{e.type}({e.source})' for e in agent_actions]}, hunger={agent.needs.hunger:.2f}, energy={agent.energy:.2f}, wood={agent.inventory.resources.get('wood',0)}")
    elif tick % 10 == 0:
        print(f"Tick {tick}: planner={decision.action}, hunger={agent.needs.hunger:.2f}, energy={agent.energy:.2f}, wood={agent.inventory.resources.get('wood',0)}")